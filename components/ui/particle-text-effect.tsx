...
export type ParticleTextEffectProps = {
  ...
  /** Logical CSS pixels; when both omitted, the parent box is measured. */
  width?: number;
  height?: number;
  interactive?: boolean;
  autoRotate?: boolean;
  /** Step in image buffer (larger = fewer particles, better performance). */
  pixelSteps?: number;
  fontFamily?: string;
  /** Target RGB for formed text (default slate-400). */
  particleColor?: { r: number; g: number; b: number };
  /** Use full clear each frame (recommended for glass / layered UI). */
  transparentTrail?: boolean;
};

const DEFAULT_SLATE: { r: number; g: number; b: number } = { r: 148, g: 163, b: 184 };
...
export function ParticleTextEffect({
  ...
  particleColor = DEFAULT_SLATE,
  transparentTrail = true,
  ...
  const hasFixedDimensions = widthProp != null && heightProp != null;
  const [measuredFallback, setMeasuredFallback] = useState({
    w: widthProp ?? 280,
    h: heightProp ?? 36,
  });

  const measured = hasFixedDimensions
    ? { w: widthProp as number, h: heightProp as number }
    : measuredFallback;
  ...
  const nextWord = useCallback(
    (
      ...
      offscreenCtx.font = `600 ${fontPx}px ${fontFamily}`;
      ...
      for (const coordIndex of coordsIndexes) {
        ...
        if (alpha > 0) {
          ...
          if (particleIndex < particles.length) {
            ...
            particle.maxSpeed = (Math.random() * 3 + 2) * Math.max(0.35, scale / 120);
            ...
            particle.closeEnoughTarget = Math.max(10, scale * 0.35);
  ...
  useEffect(() => {
    ...
    nextWord(words[0], canvas, particleColor, pixelSteps);
    ...
    const animate = () => {
      ...
      if (autoRotate && words.length > 1) {
        frameCountRef.current++;
        if (frameCountRef.current % 240 === 0) {
          wordIndexRef.current = (wordIndexRef.current + 1) % words.length;
          nextWord(words[wordIndexRef.current], c, particleColor, pixelSteps);
        }
      }
  ...
  return (
    <div ref={containerRef} className={cn('relative min-h-[1.25rem] w-full', className)}>
      <span className="sr-only">{joined}</span>
      <canvas ref={canvasRef} className={cn('block h-full w-full', canvasClassName)} aria-hidden />
    </div>
  );
}
